...
mapboxgl.accessToken =
  "pk.eyJ1IjoiMzA3MDI3MWoiLCJhIjoiY201d2RuY3VxMDluMjJscXp5ZW42MG1zeCJ9.c7fETI_inRzWjln-_ePd-w";
    ...
    .setHTML(
      `
            <h3>${
              ...
              `Unknown ${landmark.properties.historic}`
            }</h3>
            <p>Landmark Type: ${landmark.properties.historic.replace(
              /_/g,
              " "
            )}</p>
            ${
              landmark.properties.opening_hours
                ? `<p>Opening Times: ${landmark.properties.opening_hours}</p>`
                : ""
            }
            ${
              landmark.properties.website
                ? `<p>Website: <a href="${landmark.properties.website}" target="_blank" rel="noopener noreferrer">${landmark.properties.website}</a></p>`
                : ""
            }
            ${
              imageUrl
                ? `<p><img src="${imageUrl}" alt="${landmark.properties.name}" style="max-width: 200px; height: auto;" onerror="this.style.display='none';" /></p>`
                : ""
            }
        `
...
function visitRandomLandmark() {
  //Triggers selection of a random landmark when button is clicked

  categories.forEach((layer) => {
    let landmarks = map.querySourceFeatures("composite", {
      sourceLayer: layer
    });
  ...
  let randomLandmark =
    allLandmarks[Math.floor(Math.random() * allLandmarks.length)];
...
document
  .getElementById("random-landmark")
  .addEventListener("click", visitRandomLandmark); //Triggers the visitRandomLandmark function when button is clicked.
